...
class RentalBookingsPerDay:

    def __init__(self,config, bookings, rentals, labels=['car2go', 'enjoy'], save=False, name='ReB_date.pdf'):
        ...
        for label in labels:
            if isinstance(bookings, pd.DataFrame):
                ...
                df_b = date_list.merge(df_b, how='left', left_on='Date_list', right_index=True).set_index('Date_list')[
                    ref_col]
                ax.plot(df_b.index, df_b.values, color=config['colors'][label], label='%s Bookings' % label)
            ...
            if isinstance(rentals, pd.DataFrame):
                ...
                df_r = date_list.merge(df_r, how='left', left_on='Date_list', right_index=True).set_index('Date_list')[
                    ref_col]
                ax.plot(df_r.index, df_r.values, color=config['colors'][label], label='%s Rentals' % label)
